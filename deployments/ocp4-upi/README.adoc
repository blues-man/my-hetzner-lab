...
cat master.ign | jq --slurpfile contect <(cat etc-hosts.json | sed 's/core1.example.com/controller-0/')  '.storage.files |= . + $contect'


/etc/sysconfig/network-scripts/ifcfg-ens3 => $(jq -sRr @uri < bootstrap-ifcfg-ens3)
/etc/resolv.conf => echo -e "search ucp4-upi.bohne.io\nnameserver 10.84.3.1" | jq -sRr @uri
/etc/hostname => bootstrap.ucp4-upi.bohne.io
